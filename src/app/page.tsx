      ...
      <section id="new-arrivals" className={styles.section}>
        ...
        <div className={styles.productGrid}>
          {products.map((product) => {
            ...
            let buttonText = "";
            if (product.stock > 0) {
              message = `Hi Tejaswini! I would like to place an order for the *${product.name}*. How can I proceed with the payment?`;
              buttonText = "Order on WhatsApp";
            } else {
              message = `Hi Tejaswini! I see the *${product.name}* is currently out of stock. Could you please let me know when it will be available again?`;
            ...
            return (
              <div key={product.id} className={styles.productCard}>
                ...
                <div className={styles.productInfo}>
                  ...
                  <div style={{ fontSize: '0.8rem', color: product.stock < 5 && product.stock > 0 ? '#dc3545' : (product.stock === 0 ? '#666' : '#28a745'), marginBottom: '1rem', fontWeight: 600 }}>
                    {product.stock > 0 ? (product.stock < 5 ? `Only ${product.stock} left in stock!` : `${product.stock} in stock`) : 'Out of Stock'}
                  </div>
                  <a href={waLink} target="_blank" rel="noopener noreferrer" className={styles.whatsappButton} style={{ background: product.stock === 0 ? '#666' : '#25D366' }}>
